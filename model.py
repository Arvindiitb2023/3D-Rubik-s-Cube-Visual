...
class OpenGLWidget(QOpenGLWidget):
    ...
    def FB(self,axis,color):
        ...
        glColor3f(*color)
        d = 0.45  # smaller than 0.5
        glVertex3f( d,  d,  axis[2]*0.501)
        glVertex3f( d, -d,  axis[2]*0.501)
        glVertex3f(-d, -d,  axis[2]*0.501)
        glVertex3f(-d,  d,  axis[2]*0.501)
    ...
    def UD(self,axis,color):
        ...
        glColor(*color)
        d = 0.45
        glVertex3f( 0.45,  0.501*axis[1],  0.45)
        glVertex3f(-0.45,  0.501*axis[1],  0.45)
        glVertex3f(-0.45,  0.501*axis[1], -0.45)
        glVertex3f( 0.45,  0.501*axis[1], -0.45)
    
    def draw_black_cube(self):
        glColor3f(0, 0, 0)
        glBegin(GL_QUADS)
        for face in [
    ...
    def mousePressEvent(self, event):
        if self.coloring_mode and event.button() == Qt.MouseButton.LeftButton:
            x, y = event.position().x(), event.position().y()
            self.clicked_position = QPoint(int(x), int(y))
